...
describe("advanced > ", () => {
  ...
  describe("추가된 cartUtils >", () => {
    ...
    const amountCoupon: Coupon = { name: "amount", code: "AMOUNT5000", discountType: "amount", discountValue: 5000 };
    ...
    const cart: CartItem[] = [{ product: testProduct, quantity: 3 }];

    describe("applyCouponDiscount", () => {
      test("쿠폰의 타입이 amount일 때 맞는 할인이 적용됩니다,", () => {
        expect(cartUtils.applyCouponDiscount(10000, amountCoupon)).toBe(5000);
      });
      test("쿠폰의 타입이 percent일 때 맞는 할인이 적용됩니다,", () => {
        expect(cartUtils.applyCouponDiscount(10000, percentageCoupon)).toBe(9000);
      });
    });

    describe("calculateTotalCouponDiscount", () => {
      test("쿠폰의 타입이 amount일 때 맞는 할인이 적용됩니다,", () => {
        expect(cartUtils.calculateTotalCouponDiscount(10000, 10000, amountCoupon)).toEqual({
          totalAfterDiscount: 5000,
          totalDiscount: 5000,
        });
      });
      test("쿠폰의 타입이 percentage일 때 맞는 할인이 적용됩니다,", () => {
        expect(cartUtils.calculateTotalCouponDiscount(10000, 10000, percentageCoupon)).toEqual({
          totalAfterDiscount: 9000,
          totalDiscount: 1000,
        });
      });
    });
    describe("calculateTotalItemDiscount", () => {
      test("장바구니의 할인 전 총액과 할인 후 총액을 계산합니다", () => {
        expect(cartUtils.calculateTotalItemDiscount(cart)).toEqual({
          totalBefore: 300,
          totalAfter: 270,
        });
      });
